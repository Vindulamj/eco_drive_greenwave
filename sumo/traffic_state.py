...
from typing import Dict, Optional, DefaultDict, List, Tuple, Set
...
from sumo.sumo import SubscriptionManager, TRACI_VARS
from sumo.utils import get_directions, get_lane_counts, is_main_lane, get_splited_edges, get_edge_id
...
class TrafficState:
    ...
    def __init__(self, config, traci_module: traci, task_context: TaskContext, net_file: Path):
        """
        ...
        self.current_vehicles_sorted_lists: DefaultDict[str, DefaultDict[int, List[Vehicle]]] = \
            defaultdict(lambda: defaultdict(lambda: []))
        self.net_infos = defaultdict(lambda: {})
        self.junctions = get_directions(net_file)
        self.edges = sorted([edge.format(j)
                             for edge in ('{}2TL', 'TL2{}')
    ...
    def step(self):
            ...
            if vehicle not in self.completed_vehicle:
                if v_id not in sim_res['arrived_vehicles_ids']:
                    vehicle.update(
                        ...
                        veh_info[(v_id, "speed")],
                        self.fuel_emissions_models.get_fuel(vehicle,
                                                            veh_info[(v_id, "speed")],
                                                            veh_info[(v_id, "acceleration")],
                                                            veh_info[(v_id, "slope")]),
                        self.fuel_emissions_models.get_emissions(vehicle,
                                                                 veh_info[(v_id, "speed")],
                                                                 veh_info[(v_id, "acceleration")],
                                                                 veh_info[(v_id, "slope")]),
                        veh_info[(v_id, "acceleration")],
        ...
        self.current_vehicles_sorted_lists = defaultdict(lambda: defaultdict(lambda: []))
        for _, vehicle in self.current_vehicles.items():
            self.current_vehicles_sorted_lists[vehicle.platoon][vehicle.lane_index].append(vehicle)

        for _, lane_indices in self.current_vehicles_sorted_lists.items():
            for lane_index, vehicle_list in lane_indices.items():
                lane_indices[lane_index] = sorted(vehicle_list, key=lambda v: v.relative_distance, reverse=True)
    ...
    def remove_vehicle(self, vehicle: Vehicle):
        ...
        self.current_vehicles_sorted_lists[vehicle.platoon][vehicle.lane_index].remove(vehicle)
    ...
    def get_phase(self, tl_id: str) -> int:
        ...
        if not ('phases' in self.net_infos and tl_id in self.net_infos['phases']):
            self.net_infos['phases'][tl_id] = self.traci_module.trafficlight.getPhase(tl_id)
    ...
    def remaining_phase_time(self, tl_id: str) -> float:
        ...
        """
        # using net_infos as cache to avoid unnecessary calls to traci
        if not ('phases_timing' in self.net_infos and tl_id in self.net_infos['phases_timing']):
            self.net_infos['phases_timing'][tl_id] = \
                self.traci_module.trafficlight.getNextSwitch(tl_id) - self.traci_module.simulation.getTime()
    ...
    def get_leader(self, vehicle: Vehicle, side_lane: int = 0) -> Optional[Vehicle]:
        ...
        if side_lane == 0:
            veh_in_lane = self.current_vehicles_sorted_lists[vehicle.platoon][current_lane_index]
            try:
            ...
            veh_in_lane = self.current_vehicles_sorted_lists[vehicle.platoon][current_lane_index + side_lane]
            best = None
    ...
    def accel(self, vehicle: Vehicle, acc: float, use_speed_factor: bool) -> bool:
        ...
        allowed_speed = self.get_speed_limit(vehicle.lane_id) * (vehicle.speed_factor if use_speed_factor else 1)
    ...
    def get_speed_limit(self, lane_id: str) -> float:
        ...
        if not (lane_id in self.net_infos and 'max_speed' in self.net_infos[lane_id]):
            self.net_infos[lane_id]['max_speed'] = self.traci_module.lane.getMaxSpeed(lane_id)
    ...
    def get_lane_length(self, lane_id: str) -> float:
        ...
        if not (edge_id in self.net_infos and 'length' in self.net_infos[edge_id]):
            if edge_id not in self.splited_edges:
                self.net_infos[edge_id]['length'] = self.traci_module.lane.getLength(edge_id+'_0')
            else:
                self.net_infos[edge_id]['length'] = self.traci_module.lane.getLength(edge_id + '_0') + \
    ...
    def get_idm_accel(self, vehicle: Vehicle) -> float:
        ...
        if vehicle.edge_id.endswith('2TL'):
            ...
            distance_to_leader = self.get_linear_distance(leader, vehicle) + 0.1 if leader is not None else 1e-10
        else:
        ...
        if leader is None:  # no car ahead
            ...
            s_star = s0 + max(0, v * t + (v * (v - leader.speed) / (2 * np.sqrt(a * b))))

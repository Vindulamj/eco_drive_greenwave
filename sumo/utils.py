...
def is_main_lane(lane_id: str) -> bool:
    ...
    """
    return (any(junction_id == 'TL' for junction_id in get_edge_id(lane_id).split('2'))
            or (is_internal_lane(lane_id) and lane_id[1:3] == 'TL'))
...
def get_lane_counts(edges: Iterable[str], net_file: Path, include_left_turn_only_lane: bool) -> Dict[str, int]:
    """
    Returns the number of lane for each given edge.
    """
    net = ElementTree.parse(net_file).getroot()
    lane_counts = {edge.attrib['id']: len(edge)
             for edge in net.findall('edge')}

    return {
        e_id: count + (1 if include_left_turn_only_lane and e_id + '_intern' in lane_counts else 0)
        for e_id, count in lane_counts.items()
        if e_id in edges
    }


def get_green_phase_timings(origin, destination, traci_module: traci) -> Tuple[int, Tuple[float, float, float]]:
    """
    ...
    for i, link in enumerate(links):
        if len(link) > 0 and \
                get_edge_id(link[0][0]) == f'{origin}2TL' and get_edge_id(link[0][1]) == f'TL2{destination}':
            link_index = i
            break
    assert link_index > -1
...
def get_splited_edges(net_file: Path) -> List[str]:
    ...
    """
    edges = [e.getID() for e in sumolib.net.readNet(str(net_file)).getEdges(withInternal=False)]
